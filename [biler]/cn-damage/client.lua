...
Citizen.CreateThread(function()
	while true do
		...
		if isPedDrivingAVehicle() then
			vehicle = GetVehiclePedIsIn(ped, false)
			function checkTyre(speed)
				for i = 1, 7 do

					if IsVehicleTyreBurst(vehicle, i, true, 1000.0) then
			...
			checkTyre(speed)
			if healthEngineCurrent >= 400 and not checkTyre(speed) then
				local speed = GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) * 3.6
					SetVehicleMaxSpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false), 260.0)
					speed = 260.0
					--checkTyre(speed)
			end
			if healthEngineCurrent <= 400 then
				local speed = GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) * 3.6
				if speed > 80 then
					SetVehicleMaxSpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false), 22.0)
					speed = 22.0
					checkTyre(speed)
				end
			end
			if healthEngineCurrent <= 300 then
				local speed = GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) * 3.6
				if speed > 60 then
					SetVehicleMaxSpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false), 16.5)
					speed = 16.5
					print(speed)
					checkTyre(speed)
				end
			...
			if healthEngineCurrent <= 200 then
				local speed = GetEntitySpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false)) * 3.6
				if speed > 40 then
					SetVehicleMaxSpeed(GetVehiclePedIsIn(GetPlayerPed(-1), false), 11.0)
					speed = 11.0
					checkTyre(speed)
				end
			end
