...
local joinedValue = 0
RegisterCommand("politi", function(source, args, rawCommand)
	...
	if HasPedGotWeapon(GetPlayerPed(-1), weaponHash, false) then
		if not joinedPoliti then 
			joinedValue = tonumber(args[1])	
			if type(joinedValue) == 'number' then 
				if 0 > joinedValue or joinedValue > 5 then joinedValue=1 end
				addPlayerToRadio(joinedValue)
				joinedPoliti=true			
			else 
				joinedValue=1
				addPlayerToRadio(joinedValue)
			...
			removePlayerFromRadio(joinedValue)
			joinedPoliti=false
		end
	end
end)
