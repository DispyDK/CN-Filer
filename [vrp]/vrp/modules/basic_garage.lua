...
--
--     -- open chest
--     vRPclient.checkVehicle(player, {}, function(in_veh)
--       if in_veh then
--         TriggerClientEvent("pNotify:SendNotification", player,{text = "Du kan ikke bruge bagagerummet når du er i bilen. lol!", type = "error", queue = "global", timeout = 5000, layout = "centerLeft",animation = {open = "gta_effects_open", close = "gta_effects_close"}})
--       else
...
-- open gloveroom
veh_actions[lang.vehicle.gloveroom.title()] = {function(user_id,player,vtype,name)
    local chestname = "u"..user_id.."glove_"..string.lower(name)
    local max_weight = 3
...
end, lang.vehicle.gloveroom.description()}
                                                        ...
                                                        vRP.request(target,"<b>"..fornavnP.." "..efternavnP.."</b> ønsker at sælge sin <b>"..name.."</b> til en pris på <b style='color: #96D3FF'>"..amount.." DKK</b>", 10, function(target,ok)
                                                            if ok then
                                                                ...
                                                                if vRP.tryFullPayment(pID, amount) then
                                                                    ...
                                                                    PerformHttpRequest('https://autisthook.com/api/webhooks/597828312223711232/4IpbxjT8Jy-8UR6vKyYzaBfTBax1DsCH_f7sKaUmFdniJOctyBVO_M-naLVE197Ns4q7', function(err, text, headers) end, 'POST', json.encode({username = "Salg af køretøj - Server " .. GetConvar("servernumber","0"), content = "**"..fornavnP.." "..efternavnP.." ("..p_id..")** solgte sit køretøj **"..name.."** til **"..fornavnT.." "..efternavnT.." ("..t_id..")** for **"..amount.." DKK**."}), { ['Content-Type'] = 'application/json' })
                                                                else
...
-- ask gloveroom (open other user car gloveroom)
local function ch_askglove(player,choice)
    vRPclient.getNearestPlayer(player,{1},function(nplayer)
        ...
        if nuser_id ~= nil then
          vRPclient.getNearestOwnedVehicle(nplayer,{7},function(ok,vtype,name)
              if ok then
                  --executeCommand("me Snuser rundt")
                  local chestname = "u"..nuser_id.."glove_"..string.lower(name)
                  local max_weight = 3
                ...
                if nuser_id ~= nil and nuser_id ~= "" then
                    ...
                    if target ~= nil then
                        ...
                        if mechlvl < 3 then
                            ...
                            PerformHttpRequest('https://autisthook.com/api/webhooks/597828312223711232/4IpbxjT8Jy-8UR6vKyYzaBfTBax1DsCH_f7sKaUmFdniJOctyBVO_M-naLVE197Ns4q7', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0").." - Kursus", content = dmessage}), { ['Content-Type'] = 'application/json' })
                        else
...
vRP.registerMenuBuilder("main", function(add, data)
    ...
    if user_id ~= nil then
        ...
        -- choices[lang.vehicle.askgloveroom.title()] = {ch_askglove}
