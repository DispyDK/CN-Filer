                        ...
                        if amount > 0 then
                            ...
                            if new_weight <= vRP.getInventoryMaxWeight(user_id) then
                                ...
                                if vRP.tryFullPayment(user_id,amount*price) then
                                    ...
                                    PerformHttpRequest('https://autisthook.com/api/webhooks/702556990026678369/9o77fjbcITd-bzaQRpSJ60-q02_c-HZPnmL7Szi-HAzlWIbheUe5HinxjfX_Rlhy_kBT', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = "**"..user_id.."** købte **"..amount.."** stk. **"..vRP.getItemName(idname).."** - ("..dato..") fra markedet **"..gtype.."**!"}), { ['Content-Type'] = 'application/json' })

                                    if (gtype == "Politi Ammo") then
                                      PerformHttpRequest('https://autisthook.com/api/webhooks/709543004024864839/sk_1gdLdm6_JTOsT4I6V3e6GLpOLDccCvnpRTprFCdKl8Z2BS9zFKh1xEgd5U-0zuSYL', function(err, text, headers) end, 'POST', json.encode({username = "Server " .. GetConvar("servernumber","0"), content = "**"..user_id.."** købte **"..amount.."** stk. **"..vRP.getItemName(idname).."** - ("..dato..") fra markedet **"..gtype.."**!"}), { ['Content-Type'] = 'application/json' })
                                    end
