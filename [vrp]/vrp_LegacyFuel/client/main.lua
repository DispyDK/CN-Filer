		...
		if not InBlacklistedVehicle then
			if Timer then
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					local uiVehicle = GetPlayersLastVehicle()
					local uiFuel = math.ceil(round(GetVehicleFuelLevel(uiVehicle), 1))
					TriggerEvent('dispy_ui:updateFuelValue',uiFuel)
					TriggerEvent('dispy_ui:hideFuel', false)
				end
			end
			...
			if nearPump and IsCloseToLastVehicle then
				...
				if IsPedInAnyVehicle(GetPlayerPed(-1), false) then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Gå ud af køretøjet for at fylde brændstof.")
				elseif IsFueling then
					local position = GetEntityCoords(vehicle)

					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Tryk på ~g~G~w~ for at standse tankning. ~r~DKK:" .. price .. "~w~.")
				...
				elseif fuel > 95.0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Dit køretøj er allerede fyldt.")
				elseif cash <= 0 then
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Du har ikke nok penge.")
				else
					DrawText3Ds(pumpLoc['x'], pumpLoc['y'], pumpLoc['z'], "Tryk på ~g~G~w~ for at starte tankning. ~r~DKK 10.00~w~/L.")
			...
			elseif NearVehicleWithJerryCan and not nearPump and Config.EnableJerryCans then
				...
				if IsFuelingWithJerryCan then
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Tryk på ~g~G~w~ for at annullere tankning af køretøj: " .. fuel .. "%")
					...
					DrawText3Ds(coords.x, coords.y, coords.z + 0.5, "Tryk på ~g~G ~w~for at fylde køretøjet med din Jerrycan.")
			...
			Citizen.Wait(500)
			TriggerEvent('dispy_ui:hideFuel', true)
		end
	end
end)
