                    ...
                    if v2.multipledoors ~= nil and v2.multipledoors == true then
                        for k3,v3 in pairs(v2.door) do
                            if v3.range ~= nil then
                                ...
                                table.insert(doors,{locked=true,key=v2.key,permission="realestate.key",name=k,number=k2,hash=v3.hash,coords=v3.coords,range=v3.range})
                            else
                                table.insert(doors,{locked=true,key=v2.key,permission="realestate.key",name=k,number=k2,hash=v3.hash,coords=v3.coords})
                            end
                        end
                    else
                        table.insert(doors,{locked=true,key=v2.key,permission="realestate.key",key=v2.key,name=k,number=k2,hash=v.doorhash,coords=v2.door})
                    end
                    -- print(k .. k2.key)
                end
            end
